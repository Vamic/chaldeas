...
var author$project$Main$pageTitles = _List_fromArray(
	['Gilgamesh', 'Emiya (Assassin)', 'Gorp']);
var author$project$Main$ReceivePage = F2(
	function (a, b) {
		return {$: 'ReceivePage', a: a, b: b};
	});
...
var author$project$Main$requestPage = function (title) {
	return A3(
		elm$core$Basics$composeR,
		elm$http$Http$getString,
		elm$http$Http$send(
			author$project$Main$ReceivePage(title)),
		'http://grandorder.wiki/index.php?action=raw&title=' + title);
};
var elm$core$Platform$Cmd$batch = _Platform_batch;
var author$project$Main$requestAll = elm$core$Platform$Cmd$batch(
	A2(elm$core$List$map, author$project$Main$requestPage, author$project$Main$pageTitles));
var author$project$Main$init = function (_n0) {
	return _Utils_Tuple2(
		{missing: _List_Nil, pages: elm$core$Dict$empty, progress: 0},
		author$project$Main$requestAll);
};
			...
			function () {
				if (msg.b.$ === 'Ok') {
					...
					return _Utils_update(
						st,
						{
							pages: A3(elm$core$Dict$insert, title, page, st.pages),
					...
					return _Utils_update(
						st,
						{
							missing: A2(elm$core$List$cons, title, st.missing),
							progress: st.progress + 1
						});
				}
			}());
...
var author$project$Main$showIf = function (a) {
	return a ? elm$core$Maybe$Just : elm$core$Basics$always(elm$core$Maybe$Nothing);
};
var author$project$Main$total = elm$core$List$length(author$project$Main$pageTitles);
...
var author$project$Main$view = function (st) {
	var size = elm$core$Dict$size(st.pages);
	var missing = elm$core$List$isEmpty(st.missing) ? _List_Nil : _List_fromArray(
		[
			A2(
			elm$html$Html$li,
			_List_fromArray(
			...
			_List_fromArray(
				[
					...
					A2(
						elm$core$List$map,
						author$project$Site$Common$text_(elm$html$Html$li),
						st.missing))
				]))
		]);
	return A3(
			...
			[
				...
				A2(
					...
					_List_fromArray(
						[
							elm$html$Html$text(
							'Loading ' + (elm$core$String$fromInt(size) + ('/' + elm$core$String$fromInt(author$project$Main$total))))
						]))),
				A2(
				author$project$Main$showIf,
				_Utils_eq(st.progress, author$project$Main$total),
				elm$core$List$isEmpty(st.missing) ? A2(
					elm$html$Html$li,
					_List_fromArray(
						[
